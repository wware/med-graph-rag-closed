...
class HGNCParser:
    ...
    def _parse_synonyms(
        self, alias_field: Optional[str], prev_symbol_field: Optional[str]
    ) -> List[str]:
    ...
    def _create_gene(
        ...
        return Gene(
            ...
            abbreviations=(
                [symbol] if symbol.isupper() else []
            ),  # Only if symbol is all-caps
    ...
    def parse(self) -> EntityCollection:
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        gene = self._create_gene(
                            hgnc_id, symbol, name, synonyms, chromosome, entrez_id
                        )
        ...
        logger.info(
            f"✓ Parse complete: {self.genes_created} genes, {self.skipped} skipped"
        )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python hgnc_parser.py <path/to/hgnc_complete_set.tsv> [output_path]"
        )
