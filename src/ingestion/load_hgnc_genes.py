        ...
        for row in reader:
            ...
            if row.get('status') == 'Approved':
                ...
                if row.get('alias_symbol'):
                    synonyms.extend([s.strip() for s in row['alias_symbol'].split('|') if s.strip()])
                if row.get('prev_symbol'):
                    synonyms.extend([s.strip() for s in row['prev_symbol'].split('|') if s.strip()])
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print("  python -m src.ingestion.load_hgnc_genes data/reference/hgnc_complete_set.txt")
        sys.exit(1)
    ...
    print(f"      uv run python -m src.ingestion.pipeline --input-dir papers/raw --batch-size 10")
