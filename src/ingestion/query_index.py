...
def search_by_entity(entity_type: str = None, entity_text: str = None, k: int = 10):
    ...
    query = {
        "size": k,
        "query": {"nested": {"path": "entities", "query": {"bool": {"must": []}}}},
    }
    ...
    print(
        f"Found {len(response['hits']['hits'])} papers with {entity_type or 'any'} entities matching '{entity_text or 'any'}':\n"
    )
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--entity-type",
        choices=["disease", "drug", "gene", "protein"],
